...
access = "UqvpWvMgxA9oxgog3FST25Ljkzi8xP7VJLrB9jJn"
secret = "dijF8J0qt1WhtY1nwMr7xRKLdTjE26VjhDVIu4u3"
...
while True:
    try:
        ticker="KRW-MATIC" #종목코드 =========================================> 여기 입력
        now = datetime.datetime.now()
        start_time = get_start_time(ticker)
        end_time = start_time + datetime.timedelta(days=1)

        if start_time < now < end_time - datetime.timedelta(seconds=(60*60)):
            ...
            if target_price < current_price and ma15 < current_price:
                krw = get_balance("KRW")
                krw_input = krw*0.1  # =======================================> 주문금액 %

                if krw > 7000000: #최소 주문금액 및 추가 주문 막는 금액 ==========> 여기 입력
                    upbit.buy_market_order(ticker, krw_input)
                else:
                    avg_price = upbit.get_avg_buy_price(ticker)
                
                    coin = get_balance("MATIC") # ======================================> 여기 입력
                    
                    if coin > 0.1 and current_price > avg_price*1.03: # 매도조건 ===> 여기 입력
